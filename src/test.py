    ...
    if verbose:
        ...
        if mode == "multilabel":
            ...
            for name in config.CLASS_NAMES["multilabel"]:
                auroc = metrics['per_class_auroc'].get(name, float('nan'))
                per_class = metrics['per_class_metrics'].get(name, {})
                recall = per_class.get('recall', 0)
    ...
    for split in splits:
        ...
        try:
            ...
            if "accuracy" in metrics:
                ...
                results[split] = metrics['macro_auroc']
